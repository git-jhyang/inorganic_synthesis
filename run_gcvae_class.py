...
from src.networks import GraphClassCVAE
...
def main(args):
    if args.graph.startswith('att'):
        args.graph = 'attention'
    elif args.graph.startswith('conv'):
        args.graph = 'convolution'

    identifier = '{:s}_{:s}_batch_{:04d}_mdim_{:02d}_{:03d}_{:1d}_cls'.format(
    ...
    beta_schedular = cyclical_kld_annealing(args.epochs, n_cycle=5, stop=0.5)
    best_loss = 1e5
    count = 0
