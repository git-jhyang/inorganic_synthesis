...
def composition_to_feature(composit_dict, feature_type='active_composit', dtype=np.float32, enable_fraction=True):
    '''
    composit_dict = {
    ...
    if feature_type in composit_fnc:
        return eval(f'{feature_type}_feature(composit_dict, dtype, enable_fraction)')
        ...
        for ele, frac in composit_dict.items():
            if ele in exception[feature_type]:
                ...
                continue
            vec.append(np.array(elmd[feature_type][ele]) * (frac if enable_fraction else 1))
...
def active_composit_feature(composit_dict, dtype, enable_fraction=True, **kwargs):
    feat_vec = np.zeros(len(ActiveElements), dtype=dtype)
    for ele, frac in composit_dict.items():
        if frac < NEAR_ZERO: continue
        feat_vec[ActiveElements.index(ele)] = frac if enable_fraction else 1
    feat_vec /= feat_vec.sum()
    return feat_vec

def metal_composit_feature(composit_dict, dtype, enable_fraction=True, **kwargs):
    feat_vec = np.zeros(len(MetalElements) + 1, dtype=dtype)
    for ele, frac in composit_dict.items():
        feat_vec[MetalElements.index(ele) + 1] = frac if enable_fraction else 1
    if feat_vec.sum() == 0:
        feat_vec[0] = 1
    return feat_vec

def ligand_composit_feature(composit_dict, dtype, enable_fraction=True, **kwargs):
    ...
    for ele, frac in composit_dict.items():
        if ele in MetalElements:
            feat_vec[0] = 0
            continue
        feat_vec[LigandElements.index(ele) + 1] = frac if enable_fraction else 1
    return feat_vec
